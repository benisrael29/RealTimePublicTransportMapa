...
const SUBURB_BOUNDARIES_URL = 'https://data.brisbane.qld.gov.au/api/explore/v2.1/catalog/datasets/suburb-boundaries/exports/geojson?limit=-1';
const ABS_SAL_POINT_QUERY_URL = 'https://geo.abs.gov.au/arcgis/rest/services/ASGS2021/SEARCH/MapServer/16/query';
const normalizeSuburb = (s) => s
    .trim()
    .toUpperCase()
    .replace(/\s+/g, ' ')
    .replace(/[’'`]/g, '')
...
const pointForGeoJsonGeometry = (geom) => {
    if (!geom)
        return null;
    ...
    if (geom.type === 'MultiPolygon') {
        ...
        for (const poly of coords) {
            ...
            if (!best || c.areaAbs > best.areaAbs)
                best = { lon: c.x, lat: c.y, areaAbs: c.areaAbs };
        }
...
async function findSalCensusCodeForPoint(lon, lat) {
...
async function fetchQuickStatsMedians(censusCode) {
    const url = `https://www.abs.gov.au/census/find-census-data/quickstats/2021/${encodeURIComponent(censusCode)}`;
    const resp = await fetch(url, { headers: { accept: 'text/html' } });
    if (!resp.ok)
        throw new Error(`HTTP ${resp.status} for ${url}`);
    const html = await resp.text();
    const mortgageMatch = html.match(/<th[^>]*scope="row"[^>]*>\s*Median monthly mortgage repayments\s*<\/th>\s*<td[^>]*>\s*([^<]+)\s*<\/td>/i);
    const rentMatch = html.match(/<th[^>]*scope="row"[^>]*>\s*Median weekly rent(?:\s*\([^)]*\))?\s*<\/th>\s*<td[^>]*>\s*([^<]+)\s*<\/td>/i);
    return {
...
async function main() {
    ...
    const bcc = await fetchJson(SUBURB_BOUNDARIES_URL);
